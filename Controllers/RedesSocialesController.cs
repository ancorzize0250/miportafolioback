...
{
    private readonly IRedSocialService _service;

    public RedesSocialesController(IRedSocialService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] RedSocialCreateDto dto)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update(long id, [FromBody] RedSocialUpdateDto dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
