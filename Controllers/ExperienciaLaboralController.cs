...
{
    private readonly IExperienciaLaboralService _service;

    public ExperienciaLaboralController(IExperienciaLaboralService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] ExperienciaLaboralCreateDto dto)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update(long id, [FromBody] ExperienciaLaboralUpdateDto dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
